...
public class FaultStatisticsService {
    ...
    public FaultStatisticsDTO getFaultList(Date startDate, Date endDate) {
        ...
        //已完成数量
        long l2 = CollUtil.isNotEmpty(faultList) ? faultList.stream().filter(re -> InspectionConstant.FAULT_STATUS.equals(re.getStatus())).count() : 0L;
        faultStatisticsDTO.setAlreadyComplete(l2);

        //未完成数量
        long l3 = CollUtil.isNotEmpty(faultList) ? faultList.stream().filter(re -> !InspectionConstant.FAULT_STATUS.equals(re.getStatus())).count() : 0L;
        faultStatisticsDTO.setNotComplete(l3);

        //报修数量
        long l4 = CollUtil.isNotEmpty(faultList) ? faultList.stream().filter(re -> InspectionConstant.FAULT_MODE_CODE_1.equals(re.getFaultModeCode())).count() : 0L;
        faultStatisticsDTO.setRepairFault(l4);

        //自检自修数量
        long l5 = CollUtil.isNotEmpty(faultList) ? faultList.stream().filter(re -> InspectionConstant.FAULT_MODE_CODE_0.equals(re.getFaultModeCode())).count() : 0L;
        ...
        if (l1>=5){
            //截取前五个值
            List<FaultFrequencyDTO> sub = ListUtil.sub(number, 0, 5);
            //子系统
            subList(faultStatisticsDTO, sub);
        }else {
            //截取总数的值
            List<FaultFrequencyDTO> sub = ListUtil.sub(number, 0,Integer.parseInt(String.valueOf(l1)));
            //子系统
            subList(faultStatisticsDTO, sub);
        }
        return faultStatisticsDTO;
    ...
    private void subList(FaultStatisticsDTO faultStatisticsDTO, List<FaultFrequencyDTO> sub) {
        ...
        String string5 = manager.translateMajors(collect, InspectionConstant.SUBSYSTEM);
