...
public class DeviceComposeController extends BaseController<DeviceCompose, IDeviceComposeService> {
     ...
     * @return
     */
    @ApiOperation(value = "根据设备类型编码查询组件/部位树", notes = "根据设备类型编码查询组件/部位树")
    @GetMapping(value = "/queryComposeTree")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "deviceTypeCode", value = "设备类型编码", required = false, paramType = "query"),
    })
    public Result<List<DeviceComposeTreeDTO>> queryComposeTree(@RequestParam(value = "deviceTypeCode", required = false) String deviceTypeCode) {
        List<DeviceComposeTreeDTO> treeDTO= deviceComposeService.queryComposeTree(deviceTypeCode);
        return Result.OK(treeDTO);
    }
