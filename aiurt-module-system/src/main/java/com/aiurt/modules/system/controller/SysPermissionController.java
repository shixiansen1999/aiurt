...
import com.aiurt.modules.system.util.PermissionDataUtil;
...
public class SysPermissionController {
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(name = "hidden", value = "模块是否启用（false否true是（默认值0））", required = false, dataTypeClass = Integer.class)
    })
...
//	}
    ...
    public Result<?> getUserPermissionByToken(HttpServletRequest request,
                                              ...
                                              @RequestParam(name = "isApp", defaultValue = "0", required = false) Integer isApp) {
        ...
        try {
            ...
            List<SysPermission> metaList = sysPermissionService.queryByUser(loginUser.getUsername(), isApp);
            //添加首页路由
            //update-begin-author:taoyan date:20200211 for: TASK #3368 【路由缓存】首页的缓存设置有问题，需要根据后台的路由配置来实现是否缓存
            if (!PermissionDataUtil.hasIndexPage(metaList) && isApp.equals(CommonConstant.IS_APP_0)) {
            ...
            if (StringUtils.isNotBlank(compUrl) && isApp.equals(CommonConstant.IS_APP_0)) {
            ...
            query.eq("del_flag", CommonConstant.DEL_FLAG_0);
            query.eq("menu_type", CommonConstant.MENU_TYPE_2);
            if (isApp != null) {
                query.eq("is_app", isApp);
            }
    ...
    public Result<?> getPermCode() {
        try {
            ...
            // 获取当前用户的权限集合
            List<SysPermission> metaList = sysPermissionService.queryByUser(loginUser.getUsername(), null);
            // 按钮权限（用户拥有的权限集合）
