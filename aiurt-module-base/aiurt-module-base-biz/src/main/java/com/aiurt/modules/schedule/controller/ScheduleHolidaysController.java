...
public class ScheduleHolidaysController {
	...
	public Result<IPage<ScheduleHolidays>> queryPageList(ScheduleHolidays scheduleHolidays,
		...
		Result<IPage<ScheduleHolidays>> result = new Result<IPage<ScheduleHolidays>>();
		QueryWrapper<ScheduleHolidays> queryWrapper = QueryGenerator.initQueryWrapper(scheduleHolidays, req.getParameterMap());
		Page<ScheduleHolidays> page = new Page<ScheduleHolidays>(pageNo, pageSize);
		IPage<ScheduleHolidays> pageList = scheduleHolidaysService.page(page, queryWrapper);
	...
	public Result<ScheduleHolidays> add(@RequestBody ScheduleHolidays scheduleHolidays) {
		...
		try {
			scheduleHolidaysService.save(scheduleHolidays);
			result.success("添加成功！");
		} catch (Exception e) {
	...
	public Result<?> delete(@RequestParam(name="id",required=true) String id) {
		try {
			scheduleHolidaysService.removeById(id);
		} catch (Exception e) {
	...
	public Result<ScheduleHolidays> deleteBatch(@RequestParam(name="ids",required=true) String ids) {
		...
		if(ids==null || "".equals(ids.trim())) {
			...
			this.scheduleHolidaysService.removeByIds(Arrays.asList(ids.split(",")));
			result.success("删除成功!");
		}
		return result;
	}
  ...
  public ModelAndView exportXls(HttpServletRequest request, HttpServletResponse response) {
      // Step.1 组装查询条件
      QueryWrapper<ScheduleHolidays> queryWrapper = null;
      try {
          String paramsStr = request.getParameter("paramsStr");
          if (oConvertUtils.isNotEmpty(paramsStr)) {
              String deString = URLDecoder.decode(paramsStr, "UTF-8");
              ScheduleHolidays scheduleHolidays = JSON.parseObject(deString, ScheduleHolidays.class);
              queryWrapper = QueryGenerator.initQueryWrapper(scheduleHolidays, request.getParameterMap());
          }
      ...
      List<ScheduleHolidays> pageList = scheduleHolidaysService.list(queryWrapper);
      //导出文件名称
      mv.addObject(NormalExcelConstants.FILE_NAME, "schedule_holidays列表");
      mv.addObject(NormalExcelConstants.CLASS, ScheduleHolidays.class);
      mv.addObject(NormalExcelConstants.PARAMS, new ExportParams("schedule_holidays列表数据", "导出人:Jeecg", "导出信息"));
  ...
  public Result<?> importExcel(HttpServletRequest request, HttpServletResponse response) {
      ...
      for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
          ...
          try {
              List<ScheduleHolidays> listScheduleHolidayss = ExcelImportUtil.importExcel(file.getInputStream(), ScheduleHolidays.class, params);
              scheduleHolidaysService.saveBatch(listScheduleHolidayss);
              return Result.ok("文件导入成功！数据行数:" + listScheduleHolidayss.size());
          } catch (Exception e) {
              log.error(e.getMessage(),e);
