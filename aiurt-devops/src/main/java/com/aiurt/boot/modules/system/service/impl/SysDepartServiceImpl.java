...
public class SysDepartServiceImpl extends ServiceImpl<SysDepartMapper, SysDepart> implements ISysDepartService {
    ...
    @Override
    @Transactional
    public String deleteBatchWithChildren(List<String> ids) {
        ...
        for (String departId : idList) {
            ...
            List<SysUserDepart> userDepartList = sysUserDepartMapper.selectList(new QueryWrapper<SysUserDepart>().eq("dep_id", departId));
        ...
        for (String id : idList) {
            sysUserDepartMapper.delete(new QueryWrapper<SysUserDepart>().eq("dep_id", id));
            //清除编码配置表
    ...
    @Override
    public List<SysDepart> queryRootDepar() {
        //因总园也可发申请，所以部门下拉框数据为全部部门
        return baseMapper.selectList(new QueryWrapper<SysDepart>()
                .eq("del_flag", CommonConstant.DEL_FLAG_0));
//		return baseMapper.queryRootDepar();
    }
    ...
    public List<SysDepart> getDepartByArea(String area) {
        ...
        wrapper.eq("depart_name", area);
        wrapper.eq("del_flag", 0);
        SysDepart sysDepart = this.getOne(wrapper);
        wrapper = new QueryWrapper();
        wrapper.eq("parent_id", sysDepart.getId());
        wrapper.eq("del_flag", 0);
        List<SysDepart> sysDepartList = this.list(wrapper);
        sysDepartList.forEach(temp -> {
            QueryWrapper queryWrapper = new QueryWrapper();
            queryWrapper.eq("del_flag", 0);
            queryWrapper.eq("parent_id", temp.getId());
