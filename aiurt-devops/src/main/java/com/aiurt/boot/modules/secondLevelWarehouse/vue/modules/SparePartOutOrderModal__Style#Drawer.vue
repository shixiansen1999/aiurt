  ...
  export default {
    ...
    data () {
      return {
        ...
        num:{rules: [{ required: true, message: '请输入出库数量!' }]},
        warehouseCode:{rules: [{ required: true, message: '请输入仓库编号!' }]},
        delFlag:{rules: [{ required: true, message: '请输入删除状态(0.未删除 1.已删除)!' }]},
        },
        url: {
          add: "/secondLevelWarehouse/sparePartOutOrder/add",
          edit: "/secondLevelWarehouse/sparePartOutOrder/edit",
        },
      }
    },
    created () {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'materialCode','num','warehouseCode','delFlag'))
		  //时间格式化
        });
