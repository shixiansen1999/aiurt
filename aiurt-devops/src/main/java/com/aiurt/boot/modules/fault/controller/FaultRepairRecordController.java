...
public class FaultRepairRecordController {
 ...
 public Result<?> importExcel(HttpServletRequest request, HttpServletResponse response) {
    ...
    public Result<FaultRepairRecord> assign(@RequestParam String code,
                                            @RequestParam String appointUserId,
                                            @RequestParam String workType,
                                            @RequestParam(required = false) String planOrderCode,
                                            @RequestParam(required = false) String planOrderImg) {
        Result<FaultRepairRecord> result = new Result<>();
        faultRepairRecordService.assign(code, appointUserId,workType, planOrderCode, planOrderImg);
        return result;
    }
     ...
     * @return
     */
    @GetMapping(value = "/assignAgain")
    public Result<FaultRepairRecord> assignAgain(@RequestParam String code,
                                            @RequestParam String appointUserId,
                                            @RequestParam String workType,
                                            @RequestParam(required = false) String planOrderCode,
                                            @RequestParam(required = false) String planOrderImg) {
        Result<FaultRepairRecord> result = new Result<>();
        faultRepairRecordService.assignAgain(code, appointUserId,workType, planOrderCode, planOrderImg);
        return result;
    }
    ...
    public Result<FaultRepairRecord> addRecord(@RequestBody FaultRepairRecordDTO dto) {
        Result<FaultRepairRecord> result = new Result<FaultRepairRecord>();
        try {
            faultRepairRecordService.addRecord(dto);
            result.success("添加成功！");
        } catch (Exception e) {
            log.error(e.getMessage(),e);
