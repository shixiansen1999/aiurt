  ...
  export default {
    ...
    data () {
      return {
        ...
        num:{rules: [{ required: true, message: '请输入入库总数量!' }]},
        delFlag:{rules: [{ required: true, message: '请输入删除状态(0.未删除 1.已删除)!' }]},
        },
        url: {
          add: "/secondLevelWarehouse/stockInOrderLevel2/add",
          edit: "/secondLevelWarehouse/stockInOrderLevel2/edit",
        },
      }
    },
    created () {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'orderCode','warehouseCode','warehouseName','num','note','delFlag'))
		  //时间格式化
        });
