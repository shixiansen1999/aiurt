 ...
 */
@Component("PatrolStandardItemLevelTypeAndCheckHandler")
public class PatrolStandardItemLevelTypeAndCheckHandler implements RowDataSetValidationRule {
    ...
    public ValidationResult validate(List<Map<String, Column>> rows, Map<String, Column> row, Column column) {
        ...
        String hierarchyType = getData(row, "hierarchy_type");

        if (ObjectUtil.isNotEmpty(hierarchyType)) {
            if (PatrolConstant.LEVEL_TYPE_1.equals(hierarchyType) && parentLevelIsSecondary(column.getData(), rows)) {
    ...
    private boolean parentLevelIsSecondary(Object data, List<Map<String, Column>> rows) {
        return rows.stream()
                ...
                .map(Column::getData)
                .anyMatch(PatrolConstant.LEVEL_TYPE_1::equals);
    }
     ...
     *
     * @param row 行数据
     * @return 如果行为子级项目，则返回 true，否则返回 false
     */
    private boolean istwoLevelWithProject(Map<String, Column> row) {
        return PatrolConstant.LEVEL_TYPE_1.equals(getData(row, "hierarchy_type")) && isPatrolProject(row);
    }
    ...
    private boolean isChildNode(String contentColumn, List<Map<String, Column>> rows) {
        if (CollUtil.isEmpty(rows) || StrUtil.isEmpty(contentColumn)) {
            return false;
        }
     ...
     * @return 如果内容列名为其他行的子节点，则返回 true，否则返回 false
     */
    private boolean findPa(String contentColumn, List<Map<String, Column>> rows) {
        if (CollUtil.isEmpty(rows) || StrUtil.isEmpty(contentColumn)) {
            return false;
        }
        for (Map<String, Column> row : rows) {
