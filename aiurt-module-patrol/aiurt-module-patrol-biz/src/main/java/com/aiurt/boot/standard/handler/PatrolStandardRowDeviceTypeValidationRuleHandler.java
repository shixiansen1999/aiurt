 ...
 * @Description:
 */
@Component("PatrolStandardRowDeviceTypeValidationRuleHandler")
public class PatrolStandardRowDeviceTypeValidationRuleHandler implements RowValidationRule {
    @Resource
    private ISysBaseAPI sysBaseApi;

    @Override
    public ValidationResult validate(Map<String, Column> row, Column column) {
        Column appointDeviceColumn = row.get("device_type");
        if (appointDeviceColumn != null) {
            if (PatrolConstant.SHI.equals(appointDeviceColumn.getData())) {
                return validateForAppointedDevice(row, column);
            } else if (PatrolConstant.FOU.equals(appointDeviceColumn.getData())) {
